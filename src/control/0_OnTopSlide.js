...
function ComExaSilde ({ name="", close=()=>{} }) {
    ...
    if (name === "Comment") {
        ...
        urlDeleteComExa = "http://kiisenglish.php.xdomain.jp/z2214505.php?step=controlCommentDelete&userId="+data.userId+"&wordId="+data.words[now].wordId+"&commentId="
        urlAddComExa = "http://kiisenglish.php.xdomain.jp/z2214505.php?step=controlCommentAdd_Edit&userId="+data.userId+"&wordId="+data.words[now].wordId
    } else {
        data_ = data.words[now].examples
        urlDeleteComExa = "http://kiisenglish.php.xdomain.jp/z2214505.php?step=controlExampleDelete&userId="+data.userId+"&wordId="+data.words[now].wordId+"&exampleId="
        urlAddComExa = "http://kiisenglish.php.xdomain.jp/z2214505.php?step=controlExampleAdd_Edit&userId="+data.userId+"&wordId="+data.words[now].wordId
    }
...
function EditAddSlide ({name="", close = () => {}}) {
    ...
    if (name === "Edit") {
        const urlEditWord = "http://kiisenglish.php.xdomain.jp/z2214505.php?step=editWord&userId="+data.userId+"&wordId="+data.words[now].wordId
        ...
        const urlAddWordToEveryOne = "http://kiisenglish.php.xdomain.jp/z2214505.php?step=addNewWordToEveryOne&userId="+data.userId
...
function ClearSlide ({ close=() => {} }) {
    ...
    const data = useSelector(state => state.data)
    const now = useSelector(state => state.move)
    const urlDeleteWord = "http://kiisenglish.php.xdomain.jp/z2214505.php?step=clearWord&userId="+data.userId+"&wordId="+data.words[now].wordId
    const loadUserData = () => {
        fetch("http://kiisenglish.php.xdomain.jp/z2214505.php?step=1&userId="+data.userId+"&password="+data.password , { method: 'GET', mode: "no-cors" })
        .then((response) => response.json())
        .then((obj) => {dispatch(reloadData(obj)); console.log("loaded all of data for "+obj.userId)})
    ...
    const changePasswordOrId = (what) => {
        if (what === "password") {
            if (oldP !== data.password) {
                ...
                fetch("http://kiisenglish.php.xdomain.jp/z2214505.php?step=changePassword&userId="+data.userId+"&password="+data.password , { 
                    method: 'POST',
                    body: newPassword, mode: "no-cors"
                })
                .then(() => {
                    fetch("http://kiisenglish.php.xdomain.jp/z2214505.php?step=1&userId="+data.userId+"&password="+newP , { method: 'GET', mode: "no-cors" })
                    .then((response) => response.json())
                    .then((obj) => {
            ...
            fetch("http://kiisenglish.php.xdomain.jp/z2214505.php?step=changeId&userId="+data.userId+"&password="+data.password , { 
                method: 'POST',
                body: newId, mode: "no-cors"
            })
            .then(() => {
                fetch("http://kiisenglish.php.xdomain.jp/z2214505.php?step=1&userId="+newI+"&password="+data.password , { method: 'GET' , mode: "no-cors"})
                .then((response) => response.json())
                .then((obj) => {
                    dispatch(reloadData(obj))
...
function DeleteAccountSlide ({ userId, show, close=()=>{}, logout=()=>{} }) {
    ...
    const DeleAccount = () => {
        logout()
        fetch("http://kiisenglish.php.xdomain.jp/z2214505.php?step=kickUser&userId="+userId, {
